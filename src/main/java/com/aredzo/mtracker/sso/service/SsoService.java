...
public class SsoService {
    ...
    public UserTokenResponse addNewUser(String email, String password) {
        SsoUserEntity user = ssoUserRepository.save(new SsoUserEntity(email, password, Instant.now().plus(7, ChronoUnit.DAYS)));
    ...
    public ValidateTokenResponse validateToken(UUID token){
        return new ValidateTokenResponse(
                ssoTokenRepository.findByTokenAndValidByBefore(token, Instant.now()).
